...
│   ├── dishi-schema.ts    # Application tables (kitchens, menus, customers)
│   └── index.ts           # Re-exports both schemas
├── relations.ts           # Drizzle v2 relations (defineRelations for all tables)
└── client.ts              # Database client factory
```

Relations are defined separately from the schema using Drizzle v2's `defineRelations`. This lets us declare relations for Better Auth tables (user, organization) without modifying the auto-generated auth-schema file.
...
**Indexes:**
- `location` - GiST spatial (enables `ST_DWithin` for "kitchens near me")
...
- **`embedding` is nullable:** pg_vector is for future semantic search and recommendations. Embeddings are generated asynchronously (e.g. via background job after item creation). The HNSW index silently skips NULL rows, so there is zero cost until embeddings are populated.
...
**Constraints:** Unique index on `(user_id, kitchen_id)` prevents duplicate saves.
...
import { kitchenProfile } from "@repo/db/schema";
...
```typescript
import { sql } from "drizzle-orm";
import { menuItem } from "@repo/db/schema";
...
### Load kitchen with full menu (Drizzle v2 relational queries)
...
### Load user with favorites and saved locations

This was impossible with v1 relations (would have required modifying auth-schema). With v2 `defineRelations`, all relations are declared externally.
...
const customer = await db.query.user.findFirst({
  ...
  with: {
    favorites: {
      with: { kitchen: { with: { organization: true } } },
    },
    locations: true,
  },
});
