...
export function OrgMembers({ orgId }: OrgMembersProps) {
  ...
  const search = useSearch({
    from: "/dashboard/owner/kitchens/$orgId/members",
  });
  ...
  const metadata = qc.getQueryData<IdealListResponseMetadata>([
    "organizations",
    "members",
    orgId,
    1,
  ...
  // const metadata = useQuery<IdealListResponseMetadata>({
  //   queryKey: ["organizations", "members", orgId, search.page ?? 1, "metadata"],
  //   enabled: !!orgId,
  // });
    ...
    <div className="h-full w-full min-w-[90%] space-y-6 p-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => navigate({ to: `/dashboard/owner/kitchens/${orgId}` })}
          >
                  ...
                  <TableRow key={member.id}>
                    <TableCell>
                      <div className="flex items-center justify-center">
                        <RoleIcons role={member.role as BetterAuthUserRoles} />
                      </div>
                    </TableCell>
      ...
      <div className="flex items-center justify-between">
        <div className="text-muted-foreground text-sm">
          Showing page {page} of {pageCount}, total {total} members
        </div>
