    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <p className="text-muted-foreground text-sm">
          Are you sure you want to ban {user?.name ?? user?.email}? They will not be able to access
          their account.
        </p>
...
export function AdminsUnbanUserDialog({
  ...
  const [open, setOpen] = useState(false);
  const unbanMutation = useMutation(unbanUserMutationOptions);
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
      ...
      <DialogContent>
        ...
        <p className="text-muted-foreground text-sm">
          Are you sure you want to unban {user?.name ?? user?.email}? They will regain access to
          their account.
        </p>
        <div className="flex justify-end gap-2">
          <Button variant="ghost" onClick={() => setOpen(false)}>
...
export function AdminsRemoveUserDialog({
  ...
  const [open, setOpen] = useState(false);
  const removeMutation = useMutation(removeUserMutationOptions);
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="destructive" className={className} disabled={!user}>
