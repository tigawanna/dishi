...
export function AdminUsersPage() {
  const { kitchenId } = useParams({ from: "/_dashboard/kitchens/$kitchenId/users/" });
  const search = useSearch({ from: "/_dashboard/kitchens/$kitchenId/users/" });
  ...
  const query = useLiveQuery(
    (q) =>
      q
        ...
        .where(({ users }) =>
          or(ilike(users.name, `%${debouncedValue}%`), ilike(users.email, `%${debouncedValue}%`)),
        )
      ...
      <div className="mx-auto flex h-full w-full flex-col items-center justify-center p-6">
        <Empty>
          <EmptyHeader>
            ...
            <EmptyDescription>
              You haven&apos;t created any users yet. Get started by creating your first one.
            </EmptyDescription>
          </EmptyHeader>
          <EmptyContent>
            <Link to="/kitchens/$kitchenId/users/new" params={{ kitchenId }} className="flex gap-2">
              <Button>
  ...
  return (
    <div className="flex h-full w-full min-w-[90%] flex-col gap-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <Link to="/kitchens/$kitchenId/users/new" params={{ kitchenId }} className="mt-3 md:mt-0">
          <Button>
            ...
            <div className="space-y-4 p-4">
              {paginatedUsers.map((u) => (
                <Card
                  ...
                  onClick={() =>
                    navigate({
                      to: `/kitchens/$kitchenId/users/$userid`,
                      params: { kitchenId, userid: u.id },
                    })
                  }
                >
                  <CardHeader className="pb-3">
              ...
              {query.isLoading ? (
                ...
                paginatedUsers.map((u) => (
                  <TableRow
                    ...
                    onClick={() =>
                      navigate({ to: `/kitchens/$kitchenId/users/$userid`, params: { kitchenId, userid: u.id } })
                    }
                  >
                    <TableCell>
